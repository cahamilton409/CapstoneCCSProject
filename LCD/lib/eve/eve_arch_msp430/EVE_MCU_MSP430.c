...
#define MISO            BIT4                // P1.1 is SPI MISO
#define MOSI            BIT3                // P1.2 is SPI MOSI
#define PD              BIT6                // P1.3 is Power Down
#define SCLK            BIT7                // P1.4 is SPI clock
#define CS              BIT2                // P1.5 is Chip Select
...
{
    if (IFG2 & UCRXIFG){      // check if SPI receiver interrupt flag is set
...
uint8_t MCU_SPIReadWrite8(uint8_t DataToWrite)
{
    ...
    while (!(UCTXIFG));            // USCI_A0 TX (SPI) buffer ready?
    UCA0TXBUF = DataToWrite;                // Send data to FT8xx by writing data to SPI buffer
    IFG2 = UCRXIFG;                       // trigger UCA0RXIFG to receive SPI data
    __delay_cycles(100);
    return DataRead;
}
...
{
    __delay_cycles(10);
    P3OUT |= (CS);                        // CS# line high
}

// -------------------------- PD line low --------------------------------------
inline void MCU_PDlow(void)
{
    P3OUT &= (~PD);                       // PD# line low
}

// ------------------------- PD line high --------------------------------------
inline void MCU_PDhigh(void)
{
    P3OUT |= (PD);                        // PD# line high
}
